...
./agent-ledger provider profiles
./agent-ledger agent profiles
./agent-ledger agent recommend --profile codex-cli --provider openai-official --surface provider-stream --signals model,usage,cache
...
- CLI: `agent-ledger workload queue`, `agent-ledger workload claim-next --holder router-a`, plus `agent-ledger workload lease acquire|renew|release|list`.
- MCP: `ledger.workload_queue`, `ledger.claim_next_workload`, `ledger.acquire_workload_lease`, `ledger.renew_workload_lease`, `ledger.release_workload_lease`, and `ledger.workload_leases`.
...
Strict adapter fixtures are available in `examples/adapter-fixtures/` for canonical events, OpenAI Responses, OpenAI Chat Completions, Anthropic Messages, provider SSE streams, OpenTelemetry GenAI/OpenInference spans, OTLP `resourceSpans`, and A2A task snapshots. Local OpenTelemetry Collector exporter and Docker Compose smoke examples are available in `examples/otel-collector/`.
...
Run `agent-ledger config status --format markdown` or open `GET /api/config/status` to inspect a privacy-safe deployment report. It exposes bind safety, auth presence, collector path counts, pricing mode, outbound surfaces, privacy flags, and remediation hints without printing raw paths, auth tokens, API credentials, webhook URLs, machine names, authors, prompts, responses, or session ids.

Use `pricing.overrides` for enterprise contracts, relay pricing, regional multipliers, or provider-specific discounts.

The optional gateway is a local provider proxy for OpenAI-compatible Chat Completions, OpenAI Responses, and Anthropic Messages. It is disabled by default, supports OpenAI-compatible Chat Completions JSON/SSE, OpenAI Responses JSON/SSE, and Anthropic Messages JSON/SSE, reads upstream API credentials from configured environment variables, and records token usage plus audit metadata without storing request messages or response content. OpenAI Responses streaming usage is recorded from final `response.completed` events. Anthropic Messages streaming usage is merged from `message_start` and `message_delta` SSE events. For OpenAI Chat Completions streaming calls, `include_stream_usage: true` asks compatible upstreams for a final usage chunk when the client did not explicitly set `stream_options.include_usage`; set it to `false` for relays that reject that option. If a successful upstream response has no parseable usage, Agent Ledger preserves the upstream response, writes no synthetic usage row, and returns `X-Agent-Ledger-Usage-Warning`. When local budget rules are enabled and a relevant global/source/model/project rule is already in `warning` or `critical` state, gateway responses include `X-Agent-Ledger-Budget-Severity`, `X-Agent-Ledger-Budget-Rule`, and `X-Agent-Ledger-Budget-Ratio`, and Agent Ledger writes a metadata-only audit event. Automatic budget fallback is still explicit and disabled by default: set `gateway.fallback_enabled: true`, `gateway.fallback_on_budget_severity`, and `gateway.fallback_models` to rewrite only configured model requests before proxying upstream. Fallback responses include `X-Agent-Ledger-Requested-Model`, `X-Agent-Ledger-Routed-Model`, and `X-Agent-Ledger-Fallback-*` headers.
...
Local desktop notification adapters can consume the same redacted event schema through `GET /api/notifications/desktop` or `agent-ledger notify desktop`. This surface is read-only: it returns title, body, severity, workload state, pending approval, and approval route items for local tray apps or OS notification bridges, but does not send outbound traffic or write audit metadata.
...
| `GET /api/readiness` | Privacy-safe control-plane readiness report for probes, wrappers, routers, and CI |
| `GET /api/admission/check` | Privacy-safe dry-run of HTTP, CLI, or MCP operation access under current read-only/RBAC rules |
...
| `GET /api/workloads/queue` | Read-only queue claimability and lease pressure stats |
...
| `GET /api/notifications/desktop?approval_due_within=24h` | Read a redacted local desktop notification adapter payload without outbound delivery |
| `GET /api/integrations` | Privacy-safe integration capability catalog |
| `GET /api/provider-profiles` | Static provider/runtime profile catalog for OpenAI-compatible providers, Anthropic, relays, Gemini-style usageMetadata, Ollama, vLLM, LM Studio, local models, and edge runtimes |
| `GET /api/agent-profiles` | Static agent CLI/framework/wrapper/router profile catalog for Codex, Claude Code, OpenCode, Kiro, OpenClaw, Pi, MCP wrappers, A2A, OpenTelemetry, CI routers, gateways, and generic multi-agent frameworks |
| `GET /api/signal-taxonomy` | Static privacy-safe signal dictionary for mapping adapter, router, provider, and observability metadata to canonical event families |
| `GET /api/integrations/signal-coverage` | Static privacy-safe signal coverage report linking taxonomy ids to adapter kinds, provider profiles, and agent profiles |
| `GET /api/integrations/readiness` | Static privacy-safe activation readiness report for local protocols, gateways, collectors, and notification surfaces |
| `GET /api/integrations/recommendation` | Read-only advisor that maps agent/provider profiles, requested surface, and available signals to ingest, validation, privacy, and quality gates |
...
| `GET /api/integrations/conformance-matrix` | Static privacy-safe adapter conformance matrix with supported input kinds, strict fixture paths, CI commands, expected event families, and provider profile links |
...
| `POST /v1/traces` | Optional local OTLP HTTP JSON/protobuf/gzip traces receiver when `integrations.otlp_receiver.enabled` is true; responses include backpressure headers for body/span limits |
| `POST /api/otlp/v1/traces` | Same receiver under the API namespace for local reverse proxies |
| `POST /api/a2a/tasks` | Convert A2A JSON task snapshots/events into workload/run/link/context/artifact/evaluation events; discovery metadata is exposed at `GET /.well-known/agent-ledger.json#a2a` |
...
| `POST /gateway/openai/v1/responses` | Optional OpenAI Responses gateway for JSON and SSE streaming when `gateway.enabled` is true |
| `POST /gateway/anthropic/v1/messages` | Optional Anthropic Messages gateway for JSON and SSE streaming when `gateway.enabled` is true |
...
When a policy returns `require_approval`, Agent Ledger records a local pending approval request and returns its id. Admins can approve or reject it through `POST /api/policy/approvals` or `agent-ledger policy resolve`; the default quorum is one approval, and `required_approvals` enables multi-actor local approval before the original operation can be retried with `approval_id=<id>` or `X-Agent-Ledger-Approval: <id>`. Approval reuse matches action/target and, when available, the original source, model, and project so a live gateway approval cannot be reused for a different model. Policy rules can also carry `approvers`, `escalate_after`, and `escalate_to`; these are stored as local routing metadata with `due_at` and `overdue` evidence for the dashboard, webhook summaries, and enforcement reports.

## MCP Tool Surface

`agent-ledger mcp` starts a local stdio JSON-RPC tool server for agent frameworks and wrappers. The implementation is intentionally local and privacy-preserving: tools can create or close workloads, link workload dependencies, acquire/renew/release workload leases, start runs on existing workloads, append run heartbeats, check run liveness and terminal-state snapshots, read the queue probe and cursor-stable workload event feed, record tool-call metadata, context refs, hashed artifacts, and quality/evaluation signals, ask for advisory policy decisions, list and vote on local approval requests, read approval route rollups, query local budget state, explain cost, and find similar workloads. Resources expose metadata-only schema, integration, budget, workload, queue, privacy-safe leases, run liveness, feed, terminal-state, and policy context; resource URIs support query parameters for scoped reads such as `agent-ledger://workloads/queue?source=codex&team=infra`, `agent-ledger://workloads/leases?include_inactive=1&limit=25`, `agent-ledger://workload/state?workload_id=...&max_age=10m`, `agent-ledger://workload/timeline?workload_id=...&limit=50`, `agent-ledger://agent-runs/liveness?max_age=10m&stale_only=1&limit=25`, `agent-ledger://workloads/feed?severity=warning&source=codex&project=agent-ledger&limit=50`, `agent-ledger://policy/approvals?status=pending&privacy=1`, and `agent-ledger://policy/approval-routes?due_within=24h&privacy=1`. `resources/subscribe` watches the exact subscribed URI locally and emits `notifications/resources/updated` with the latest cursor/hash when that scoped resource changes; volatile `generated_at` and `age_seconds` fields are ignored for hash cursors so poll-based subscriptions do not wake routers when only render time changes. Prompts provide reusable workload/cost-review/evidence templates. It does not read prompt content and does not send data to a remote MCP host by itself. MCP, REST, and CLI policy evaluation share the same local evaluator so advisory decisions are consistent across integrations.
...
`GET /api/integrations`, `GET /.well-known/agent-ledger.json`, `agent-ledger integrations`, MCP `ledger.discovery`, MCP `ledger.integrations`, and `agent-ledger://discovery/manifest` expose runtime capability fields: `writes_local_state`, `available_in_read_only`, and `runtime_status`. The discovery manifest also exposes first-class `contract_bundle_uri`, `openapi_uri`, `capability_catalog_hash`, `provider_profiles_uri`, `provider_profiles_hash`, `agent_profiles_uri`, `agent_profiles_hash`, `signal_taxonomy_uri`, `signal_taxonomy_hash`, `signal_coverage_uri`, `signal_coverage_hash`, `integration_readiness_uri`, `integration_readiness_hash`, `integration_recommendation_uri`, `integration_recommendation_hash`, `conformance_matrix_uri`, `conformance_matrix_hash`, `runtime_status_uri`, `canonical_schema_uri`, `canonical_schema_hash`, `event_examples_uri`, `adapter_spec_uri`, `adapter_spec_hash`, and `adapter_conformance_uri` fields for lightweight wrappers. `GET /api/provider-profiles`, `agent-ledger provider profiles`, MCP `ledger.provider_profiles`, and `agent-ledger://integrations/provider-profiles` expose a static, privacy-safe provider/runtime profile catalog for OpenAI-compatible providers, Anthropic, OpenRouter-style relays, LiteLLM proxies, Gemini-style `usageMetadata`, Ollama, vLLM, LM Studio, local models, and edge runtimes. `GET /api/agent-profiles`, `agent-ledger agent profiles`, MCP `ledger.agent_profiles`, and `agent-ledger://integrations/agent-profiles` expose a static, privacy-safe agent framework profile catalog for local CLIs, wrappers, routers, protocol adapters, observability bridges, and generic multi-agent frameworks. `GET /api/signal-taxonomy`, `agent-ledger signals`, MCP `ledger.signal_taxonomy`, and `agent-ledger://integrations/signal-taxonomy` expose the static signal dictionary that maps adapter, router, provider, and observability metadata to canonical event families, recommended fields, precision labels, privacy classes, and quality gates. `GET /api/integrations/readiness`, `agent-ledger integrations readiness`, MCP `ledger.integration_readiness`, and `agent-ledger://integrations/readiness` expose explicit activation gates, runtime flags, risk levels, evidence, and actions for protocol, gateway, collector, and notification surfaces before operators enable preview integrations. `GET /api/integrations/recommendation`, `agent-ledger agent recommend`, MCP `ledger.integration_recommendation`, and `agent-ledger://integrations/recommendation` return a read-only advisor report that maps an agent profile, provider profile, requested surface, and available signals to the recommended ingest path, fallback paths, strict CI command, privacy checklist, quality gates, expected event types, missing signals, and related contract hashes. `GET /api/integrations/conformance-matrix`, `agent-ledger adapter matrix`, MCP `ledger.conformance_matrix`, and `agent-ledger://integrations/conformance-matrix` expose the static adapter conformance matrix used by wrappers and CI to pick a decoder kind, fixture family, strict command, and expected metadata event family. `GET /api/contracts`, `agent-ledger contracts`, MCP `ledger.contracts`, and `agent-ledger://contracts/bundle` expose a one-shot contract bundle with document URIs, hashes, cache semantics, CLI commands, and MCP entrypoints. `GET /api/contracts/verify`, `agent-ledger contracts verify`, MCP `ledger.contracts_verify`, and `agent-ledger://contracts/verification` expose a machine-readable self-check report that validates discovery, bundle, OpenAPI, schema, adapter, runtime, read-only, and privacy invariants. `GET /api/openapi.json`, `agent-ledger openapi`, MCP `ledger.openapi`, and `agent-ledger://contracts/openapi` expose a metadata-only OpenAPI 3.1 document for stable REST control-plane endpoints. `GET /api/integrations/adapter-spec`, `agent-ledger adapter spec`, MCP `ledger.adapter_contract`, and `agent-ledger://integrations/adapter-contract` expose the same machine-readable adapter contract. `GET /api/runtime/status` and `agent-ledger runtime` provide the same process-level observer/control-plane status for probes. `GET /api/config/status`, `agent-ledger config status`, MCP `ledger.config_status`, and `agent-ledger://config/status` provide a privacy-safe deployment configuration report for wrappers, CI, and operator checks. `GET /api/readiness`, `agent-ledger readiness`, MCP `ledger.readiness`, and `agent-ledger://readiness` provide a privacy-safe readiness report that summarizes database, config, runtime, contract, ingestion, and pricing evidence without leaking local data. `GET /api/admission/check`, `agent-ledger admission check`, MCP `ledger.admission_check`, and `agent-ledger://admission/check` let wrappers and routers dry-run HTTP, CLI, and MCP operations against the current role/read-only rules before attempting a mutation. The REST discovery, contract bundle, contract verification, OpenAPI, catalog, provider profiles, agent profiles, signal taxonomy, signal coverage, integration readiness, integration recommendation, conformance matrix, runtime status, config status, readiness, admission, adapter spec, and event schema endpoints emit strong `ETag` headers and honor `If-None-Match` with `304 Not Modified`, so wrappers can revalidate contracts without reparsing unchanged JSON. Agent routers and wrappers should use these fields instead of hardcoding endpoint assumptions, especially when `rbac.read_only` is enabled.

Signal coverage is available as `signal_coverage_uri` and `signal_coverage_hash` in discovery, `GET /api/integrations/signal-coverage` over REST, `agent-ledger signal-coverage` or `agent-ledger signals coverage` over CLI, MCP `ledger.signal_coverage`, and `agent-ledger://integrations/signal-coverage`. It verifies adapter `required_signals` against canonical taxonomy ids and reports coverage gaps before new adapters are enabled.

Integration readiness is available as `integration_readiness_uri` and `integration_readiness_hash` in discovery, `GET /api/integrations/readiness`, `agent-ledger integrations readiness` or `agent-ledger integration-readiness`, MCP `ledger.integration_readiness`, and `agent-ledger://integrations/readiness`. It reports activation state, runtime flags, explicit enablement gates, risk level, evidence, and actions for current and future agent ecosystem surfaces without reading local usage rows.
...
- `agent-ledger://workloads/queue` with read-only queue claimability and lease pressure stats; supports `source`, `project`, `repo`, `team`, `owner`, `status`, and `q`
- `agent-ledger://workloads/leases` with privacy-safe lease rows for router context; supports `include_inactive` and `limit`; omits workload id, holder, purpose, and lease tokens
...
- `agent-ledger://workload/state` with a privacy-safe single-workload terminal-state snapshot; requires `workload_id`, supports `max_age`, and omits workload id, goal, project, repo, branch, team, owner, and status messages
- `agent-ledger://workload/timeline` with privacy-safe single-workload timeline event kinds; requires `workload_id`, supports `limit`, and omits event ids, run ids, labels, details, goals, project metadata, status messages, and raw identifiers
- `agent-ledger://agent-runs/liveness` with privacy-safe run liveness rows for async monitors; supports `max_age`, `stale_only`, `source`, `project`, and `limit`; omits run id, workload id, goal, project, repo, branch, and status message
...
Canonical event ingest supports workload, workload-link, run, run-heartbeat, model-call, tool-call, context-ref, artifact, evaluation, and policy-decision events. Payloads are metadata-only; raw prompt/content keys are rejected instead of silently persisted. The current event-envelope contract accepts only `schema_version: "v1"`; unknown versions fail validation explicitly. `GET /api/event-examples`, `agent-ledger event examples`, and MCP `ledger.event_examples` return privacy-safe templates for every supported event type. `GET /api/integrations/adapter-spec`, `agent-ledger adapter spec`, MCP `ledger.adapter_contract`, and `agent-ledger://integrations/adapter-contract` expose a machine-readable adapter contract with supported input kinds, required envelope fields, forbidden payload keys, token semantics, quality gates, validation commands, and ingest entrypoints. `GET /api/integrations/conformance-matrix`, `agent-ledger adapter matrix`, MCP `ledger.conformance_matrix`, and `agent-ledger://integrations/conformance-matrix` expose fixture families, strict CI commands, expected event types, and provider profile links without storing prompt or response content. `GET /api/agent-profiles`, `agent-ledger agent profiles`, MCP `ledger.agent_profiles`, and `agent-ledger://integrations/agent-profiles` map current and future agent CLIs, frameworks, wrappers, routers, protocol adapters, and observability bridges to the recommended local ingest surface. `POST /api/events/validate` and `agent-ledger event validate` run the same contract checks without writing to SQLite, which is useful for direct canonical event checks. `POST /api/integrations/conformance` and `agent-ledger adapter conformance` also convert provider JSON, provider SSE, OpenTelemetry GenAI, and A2A fixtures before validation, so wrapper CI can prove compatibility before enabling ingest. The envelope also carries adapter provenance fields such as `schema_version`, `source_version`, `parser_version`, `raw_ref`, and `match_type`, so future collectors can explain whether an event was exact, estimated, reconstructed, source-reported, or fuzzy without storing prompt content. Canonical `model.call` events are also projected into `usage_records` so dashboard, budget, export, and preflight APIs use one token source where possible. `workload.linked` records dependency and lineage edges for async goal graphs without storing prompt content. `agent.run.heartbeat` appends liveness/progress rows and updates the run snapshot so long-running async agents can be monitored without reading prompts; liveness queries identify active runs whose heartbeat or start time is older than a configured threshold. `GET /api/integrations`, `agent-ledger integrations`, and `ledger.integrations` expose the current connector/protocol capability catalog without leaking local source paths. `POST /api/otel/genai` and `agent-ledger otel ingest` accept OpenTelemetry GenAI JSON spans and persist only selected metadata/token fields. When explicitly enabled, `POST /v1/traces` and `POST /api/otlp/v1/traces` accept uncompressed or `Content-Encoding: gzip` OTLP HTTP JSON/protobuf trace batches with body and span-count limits; compressed and decoded bodies are bounded, accepted and rejected requests expose `X-Agent-Ledger-OTLP-*` backpressure headers plus a `backpressure` JSON object with body bytes, span counts, configured limits, and produced events, and over-limit rejects write metadata-only audit rows. Optional OTLP/gRPC TraceService ingest is also available behind `integrations.otlp_receiver.grpc_enabled`; it is disabled by default, refuses non-loopback bind addresses, and stores only metadata-derived GenAI usage events. `POST /api/a2a/tasks` and `agent-ledger a2a ingest` accept A2A task snapshots/events and persist task lifecycle, delegated parent/child lineage, context, evidence-reference hash, artifact, evaluation, and policy metadata while excluding message/history/artifact-part content. `GET /.well-known/agent-ledger.json#a2a` exposes privacy-safe A2A discovery metadata for clients and routers; Agent Ledger is a local telemetry adapter, not a full A2A task execution server. `POST /api/provider/calls` and `agent-ledger provider ingest` accept OpenAI-compatible, Anthropic-style, and LiteLLM-style usage envelopes while excluding request/response message content. When explicitly enabled, `POST /gateway/openai/v1/chat/completions` proxies OpenAI-compatible JSON or SSE streaming requests in memory, `POST /gateway/openai/v1/responses` proxies OpenAI Responses JSON or SSE streaming requests in memory, and `POST /gateway/anthropic/v1/messages` proxies Anthropic Messages JSON or SSE streaming requests in memory; all apply local policy checks, write usage/audit metadata, and expose metering state through response headers without storing prompt or response content. `POST /api/reconciliation/import` and `agent-ledger reconcile import` accept local provider CSV/JSON billing exports, store only summary totals, statement hash, window, and warnings, and compare them with the local ledger for the same window.

Provider usage adapters also accept `usage_metadata`/`usageMetadata` relay envelopes, generic SSE usage metadata fixtures, and request/response metadata wrappers. Wrapper conversion uses a whitelist: request/response bodies, headers, request content, generation content, and secrets are ignored; provider bill or invoice refs are hashed into reconciliation context references. `GET /api/provider-profiles` documents the expected usage schemas, pricing strategy, reconciliation support, privacy notes, and conformance fixture paths for official providers, relays, local runtimes, and edge runtimes. `POST /api/provider/calls` returns post-ingest budget advisories and reconciliation hook counts without blocking local imports.
...
- The optional provider gateway is disabled by default. It forwards prompt content only to the configured upstream in memory, reads API credentials from environment variables, and stores usage metadata rather than message content. Budget fallback is also disabled by default and rewrites only explicitly mapped models when configured thresholds are already breached.
...
Implemented foundation: canonical workload schema, metadata-only canonical event ingest, machine-readable adapter contract, adapter conformance matrix, provider/runtime profile catalog, workload dependency/lineage links, retry-safe control operation idempotency for workload/run writes, short-lived workload leases for async execution claims, async run start/heartbeat/liveness ledger, derived workload terminal-state snapshots and local workload event feed/SSE stream, explicit workload evaluation signals, disabled-by-default redacted workload and approval webhook notifications, read-only local desktop notification adapter payloads, privacy-safe discovery manifest, contract bundle index, OpenAPI control-plane contract, runtime status probe, privacy-safe config status probe, control-plane readiness probe, operation admission dry-run, canonical-to-usage projection plus repair, OpenTelemetry GenAI JSON span mapping, optional local OTLP HTTP JSON/protobuf/gzip traces receiver with strict `resourceSpans` fixture, optional loopback-only OTLP/gRPC TraceService receiver, collector exporter and compose smoke examples, and per-request backpressure response/audit metrics, A2A task telemetry mapping, provider usage envelope mapping, optional local OpenAI-compatible Chat Completions JSON/SSE, OpenAI Responses JSON/SSE, and Anthropic Messages JSON/SSE gateway, provider bill reconciliation import, model router simulation, preflight cost estimates, session cost replay, repo cost badges, integration capability catalog, signed offline bundle export/import, legacy session backfill, workload API, workload CSV export, local policy approval requests, quorum-based approval votes, approval routing/escalation metadata, approval route summaries and enforcement evidence, CLI workload/event/policy/router/replay/badge/preflight/projection/config/readiness/admission commands, CLI run wrapper, and local MCP stdio tools/resources/resource-subscriptions/prompts.

Planned integrations: provider-native gateway adapters, Postgres team mode, OIDC/SSO, native MCP subscription transport when host clients support it, and external approval notification adapters.
