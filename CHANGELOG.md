...
- Conservative goal completion audit inside goal coverage plus `agent-ledger goal audit`, separating implementation coverage from completion and requiring every local blocker, experimental surface, and remaining item to be closed before the persistent product goal can be marked complete.
...
- Read-only `agent-ledger ui check` static UI/UX contract for the embedded black/white/gray dashboard, covering local assets, accessibility, chart summaries, privacy entrypoints, monochrome palette, a structured 375/768/1024/1440px viewport matrix, touch targets, skeleton reduced-motion, stable chart panels, and contained table overflow.
- Embedded dashboard integration readiness panel and KPI backed by `/api/integrations/readiness`, showing local-first runtime posture, activation counts, disabled-by-config surfaces, review-required surfaces, and top capability gates without storing prompt or response content.
...
- Signal coverage report through `GET /api/integrations/signal-coverage`, `agent-ledger signal-coverage`, `agent-ledger signals coverage`, MCP `ledger.signal_coverage`, and `agent-ledger://integrations/signal-coverage`, validating adapter required signals against canonical taxonomy ids and linking them to provider and agent profiles.
- Integration readiness report through `GET /api/integrations/readiness`, `agent-ledger integrations readiness`, MCP `ledger.integration_readiness`, and `agent-ledger://integrations/readiness`, exposing privacy-safe activation gates, runtime flags, risk levels, evidence, and actions before preview protocol, gateway, collector, or notification surfaces are enabled.
