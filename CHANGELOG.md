...
- Agent framework profile catalog through `GET /api/agent-profiles`, `agent-ledger agent profiles`, MCP `ledger.agent_profiles`, `agent-ledger://integrations/agent-profiles`, discovery, contract bundle, OpenAPI, goal coverage, and contract verification, covering Codex, Claude Code, OpenCode, Kiro, OpenClaw, Pi, MCP wrappers, A2A, OpenTelemetry, CI routers, gateways, and generic multi-agent frameworks without storing prompt or response content.
- Signal taxonomy catalog through `GET /api/signal-taxonomy`, `agent-ledger signals`, MCP `ledger.signal_taxonomy`, `agent-ledger://integrations/signal-taxonomy`, discovery, contract bundle, OpenAPI, goal coverage, and contract verification, mapping integration signals to canonical event families, recommended fields, precision labels, privacy classes, and quality gates.
- Integration recommendation advisor through `GET /api/integrations/recommendation`, `agent-ledger agent recommend`, MCP `ledger.integration_recommendation`, and `agent-ledger://integrations/recommendation`, mapping agent/provider profiles, surfaces, and available signals to recommended ingest paths, strict CI commands, privacy checklists, quality gates, expected events, missing signals, and contract hashes without reading or writing usage rows.
- Adapter conformance matrix through `GET /api/integrations/conformance-matrix`, `agent-ledger adapter matrix`, MCP `ledger.conformance_matrix`, `agent-ledger://integrations/conformance-matrix`, discovery, contract bundle, OpenAPI, and contract verification, listing privacy-safe fixture families, strict CI commands, expected canonical event types, and provider profile links.
- Matrix-driven adapter fixture CI now verifies every conformance matrix fixture path exists, passes strict conformance, emits the declared canonical event types, and has reciprocal provider profile links where applicable.
...
- OpenAPI now advertises `405 Method Not Allowed` responses with `Allow` header contracts for GET-only, POST-only, and GET/POST mixed endpoints.
...
- Added session replay/detail and scoped insight-event indexes for large ledgers with many sessions, anomaly rows, and watchdog rows.
...
- Optional local OpenAI-compatible gateway through `POST /gateway/openai/v1/chat/completions`, gated by `gateway.enabled`, supporting JSON and SSE streaming responses, using environment-variable API credentials, local policy checks, usage response metering, and audit metadata without storing prompt or response content.
- Optional local OpenAI Responses gateway through `POST /gateway/openai/v1/responses`, gated by `gateway.enabled`, supporting JSON and SSE streaming responses, environment-variable API credentials, local policy checks, usage response metering from final `response.completed` events, and audit metadata without storing prompt or response content.
- Optional local Anthropic Messages gateway through `POST /gateway/anthropic/v1/messages`, gated by `gateway.enabled`, supporting non-streaming JSON responses, environment-variable API credentials, local policy checks, usage response metering, and audit metadata without storing prompt or response content.
