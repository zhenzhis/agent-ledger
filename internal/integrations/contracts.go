	...
	bundle := ContractBundle{
		...
		Documents: []ContractDocument{
			...
			},
			{
				...
				Version:          "v1",
				Hash:             GoalCoverageContractFingerprint(opts, runtime),
				PrimaryURI:       "/api/goal-coverage",
				HTTPMethods:      []string{"GET"},
				CLICommands:      []string{"agent-ledger goal coverage"},
				MCPTools:         []string{"ledger.goal_coverage"},
				MCPResources:     []string{"agent-ledger://goal/coverage"},
				Revalidation:     "ETag + If-None-Match",
				Privacy:          "requirement-level evidence, hashes, endpoint names, command names, docs, tests, and external dependency notes only",
				ReadOnlySafe:     true,
				WritesLocalState: false,
			},
			{
			...
			},
			{
				...
				Version:          "v1",
				Hash:             ProviderProfilesFingerprint(),
			...
			},
			{
				...
				Version:          "v1",
				Hash:             IntegrationReadinessFingerprint(opts),
				PrimaryURI:       "/api/integrations/readiness",
				HTTPMethods:      []string{"GET"},
				CLICommands:      []string{"agent-ledger integrations readiness", "agent-ledger integration-readiness"},
				MCPTools:         []string{"ledger.integration_readiness"},
				MCPResources:     []string{"agent-ledger://integrations/readiness"},
				Revalidation:     "ETag + If-None-Match",
				Privacy:          "runtime capability activation metadata only",
				ReadOnlySafe:     true,
				WritesLocalState: false,
			},
			{
				...
				Version:          "v1",
				Hash:             IntegrationSmokeFingerprint(opts, runtime),
				PrimaryURI:       "/api/integrations/smoke",
				HTTPMethods:      []string{"GET"},
				CLICommands:      []string{"agent-ledger integrations smoke", "agent-ledger integration-smoke"},
				MCPTools:         []string{"ledger.integration_smoke"},
				MCPResources:     []string{"agent-ledger://integrations/smoke"},
				Revalidation:     "ETag + If-None-Match",
				Privacy:          "metadata-only contract, conformance, signal, readiness, and recommendation smoke evidence",
				ReadOnlySafe:     true,
				WritesLocalState: false,
			},
			{
			...
			{
				ID:               "integration-rollout-plan",
				Name:             "Integration Rollout Plan",
				Contract:         "agent-ledger.integration-rollout-plan",
				Version:          "v1",
				Hash:             IntegrationRolloutFingerprint(IntegrationRolloutRequest{}),
				PrimaryURI:       "/api/integrations/rollout-plan",
				HTTPMethods:      []string{"GET"},
				CLICommands:      []string{"agent-ledger integrations rollout-plan", "agent-ledger integration-rollout"},
				MCPTools:         []string{"ledger.integration_rollout_plan"},
				MCPResources:     []string{"agent-ledger://integrations/rollout-plan"},
				Revalidation:     "ETag + If-None-Match",
				Privacy:          "static rollout checklist metadata, fixture paths, validation commands, admission checks, release gates, and rollback hints only",
				ReadOnlySafe:     true,
				WritesLocalState: false,
			},
			{
				ID:               "integration-evidence-kit",
				Name:             "Integration Evidence Kit",
				Contract:         "agent-ledger.integration-evidence-kit",
				Version:          "v1",
				Hash:             IntegrationEvidenceKitFingerprint(opts, runtime, IntegrationEvidenceKitRequest{}),
				PrimaryURI:       "/api/integrations/evidence-kit",
				HTTPMethods:      []string{"GET"},
				CLICommands:      []string{"agent-ledger integrations evidence-kit", "agent-ledger integration-evidence-kit"},
				MCPTools:         []string{"ledger.integration_evidence_kit"},
				MCPResources:     []string{"agent-ledger://integrations/evidence-kit"},
				Revalidation:     "ETag + If-None-Match",
				Privacy:          "static release evidence metadata, hashes, CI commands, review lanes, fixture paths, and rollout gates only",
				ReadOnlySafe:     true,
				WritesLocalState: false,
			},
			{
			...
			{
				ID:               "integration-lockfile",
				Name:             "Integration Lockfile",
				Contract:         "agent-ledger.integration-lockfile",
				Version:          "v1",
				Hash:             IntegrationLockfileOpenAPIFingerprint(opts, runtime),
				PrimaryURI:       "/api/integrations/lockfile",
				HTTPMethods:      []string{"GET"},
				CLICommands:      []string{"agent-ledger integrations lockfile", "agent-ledger integration-lockfile"},
				MCPTools:         []string{"ledger.integration_lockfile"},
				MCPResources:     []string{"agent-ledger://integrations/lockfile"},
				Revalidation:     "ETag + If-None-Match",
				Privacy:          "static hash baseline metadata, refresh commands, and redaction rules only",
				ReadOnlySafe:     true,
				WritesLocalState: false,
			},
			{
				ID:               "integration-upgrade-gate",
				...
				PrimaryURI:       "/api/integrations/upgrade-gate",
				HTTPMethods:      []string{"GET"},
				CLICommands:      []string{"agent-ledger integrations upgrade-gate", "agent-ledger integration-upgrade-gate"},
				MCPTools:         []string{"ledger.integration_upgrade_gate"},
				MCPResources:     []string{"agent-ledger://integrations/upgrade-gate"},
				Revalidation:     "ETag + If-None-Match",
				Privacy:          "static release gate decision metadata, CI commands, and redaction rules only",
				ReadOnlySafe:     true,
				WritesLocalState: false,
			},
			{
				...
				Version:          "v1",
				Hash:             SchemaEvolutionGateOpenAPIFingerprint(),
				PrimaryURI:       "/api/schema/evolution-gate",
				HTTPMethods:      []string{"GET"},
				CLICommands:      []string{"agent-ledger schema-gate", "agent-ledger event schema-gate", "agent-ledger integrations schema-gate"},
				MCPTools:         []string{"ledger.schema_evolution_gate"},
				MCPResources:     []string{"agent-ledger://schema/evolution-gate"},
				Revalidation:     "ETag + If-None-Match",
				Privacy:          "static canonical schema compatibility metadata, event type ids, rejected key ids, CI commands, and redaction rules only",
				ReadOnlySafe:     true,
				WritesLocalState: false,
			},
			{
...
func ContractVerificationReportFor(opts Options, runtime *storage.RuntimeStatus) ContractVerificationReport {
	...
	integrationReadiness := IntegrationReadiness(opts)
	integrationSmoke := IntegrationSmokeReportFor(opts, runtime)
	...
	privacyLanguageOK, privacyLanguageActual := contractPublicPrivacyLanguageStatus(map[string]interface{}{
		...
		"integration_lockfile": map[string]interface{}{
			"contract": "agent-ledger.integration-lockfile",
			"version":  "v1",
			"privacy":  "metadata-only lockfile witness; excludes prompts, responses, sessions, secrets, local paths, machine names, authors, and webhook URLs",
			"hash":     IntegrationLockfileOpenAPIFingerprint(opts, runtime),
		},
		"integration_upgrade_gate": map[string]interface{}{
			"contract": "agent-ledger.integration-upgrade-gate",
			"version":  "v1",
		...
		"schema_evolution_gate": map[string]interface{}{
			"contract": "agent-ledger.schema-evolution-gate",
			"version":  "v1",
			"privacy":  "metadata-only schema compatibility witness; excludes prompts, responses, sessions, secrets, local paths, machine names, authors, and webhook URLs",
			"hash":     SchemaEvolutionGateOpenAPIFingerprint(),
		},
	...
	requiredDocs := []struct {
		...
		{id: "goal-coverage", hash: GoalCoverageContractFingerprint(opts, runtime)},
