...
func Registry(opts Options) Catalog {
	...
	capabilities := []Capability{
		...
		},
		{
			...
			Category:       "gateway",
			Protocol:       "OpenAI-compatible Chat Completions HTTP JSON/SSE, OpenAI Responses HTTP JSON/SSE, and Anthropic Messages HTTP JSON proxy",
			...
			Limitations:    []string{"disabled by default", "OpenAI Chat Completions streaming usage requires upstream final usage chunks; gateway requests them by default when compatible", "OpenAI Responses streaming records usage from response.completed events", "Anthropic Messages gateway supports non-streaming JSON responses first", "does not persist prompt or response content"},
