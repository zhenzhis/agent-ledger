...
func Registry(opts Options) Catalog {
	...
	capabilities := []Capability{
		...
		},
		{
			...
			Category:       "gateway",
			Protocol:       "OpenAI-compatible, Anthropic-style, LiteLLM-style, and usageMetadata relay usage JSON",
			Direction:      "ingest",
			Status:         "implemented",
			Maturity:       "local-preview",
			Enabled:        true,
		...
		},
		{
			...
			Category:       "gateway",
			Protocol:       "OpenAI-compatible Chat Completions HTTP JSON/SSE, OpenAI Responses HTTP JSON/SSE, Anthropic Messages HTTP JSON/SSE proxy, and generic relay SSE fixture conformance",
			...
			NextMilestones: []string{"budget-aware automatic model fallback policies"},
		},
		{
