...
func Registry(opts Options) Catalog {
	...
	capabilities := []Capability{
		...
		},
		{
			...
			Category:       "protocol",
			Protocol:       "A2A JSON task snapshots/events with delegated lineage and evidence references",
			Direction:      "ingest",
			Status:         "implemented",
			Maturity:       "local-preview",
			Enabled:        true,
			...
			Limitations:    []string{"local JSON mapper, not a full A2A server", "parent workload placeholders are reconstructed when delegated task metadata references a parent not present in the local ledger", "does not persist A2A message/history content"},
			NextMilestones: []string{"add A2A server discovery metadata"},
		},
		{
