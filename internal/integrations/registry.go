...
func Registry(opts Options) Catalog {
	...
	capabilities := []Capability{
		...
		},
		{
			...
			Category:       "gateway",
			Protocol:       "OpenAI-compatible HTTP JSON/SSE and Anthropic Messages HTTP JSON proxy",
			...
			Endpoints:      []string{"POST /gateway/openai/v1/chat/completions", "POST /gateway/anthropic/v1/messages"},
			Limitations:    []string{"disabled by default", "OpenAI streaming usage requires upstream final usage chunks; gateway requests them by default when compatible", "Anthropic Messages gateway supports non-streaming JSON responses first", "does not persist prompt or response content"},
			NextMilestones: []string{"Anthropic SSE usage capture", "budget-aware routing decisions", "streaming conformance fixtures for more providers"},
		},
		{
