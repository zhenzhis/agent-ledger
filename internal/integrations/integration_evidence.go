...
type IntegrationEvidenceKitHashes struct {
	...
	IntegrationProductionGateHash string `json:"integration_production_gate_hash"`
...
func IntegrationEvidenceKitFor(opts Options, runtime *storage.RuntimeStatus, req IntegrationEvidenceKitRequest) IntegrationEvidenceKitReport {
	...
	hashes := IntegrationEvidenceKitHashes{
		...
		IntegrationProductionGateHash: IntegrationProductionGateOpenAPIFingerprint(opts, runtime),
...
func integrationEvidenceItems(rollout IntegrationRolloutPlanReport, hashes IntegrationEvidenceKitHashes) []IntegrationEvidenceItem {
	items := []IntegrationEvidenceItem{
		...
		integrationEvidenceItem("integration-production-gate", "release", "Integration production gate", "gate", "agent-ledger integrations production-gate --strict", "GET /api/integrations/production-gate", "ledger.integration_production_gate", "agent-ledger://integrations/production-gate", hashes.IntegrationProductionGateHash, true, "production gate decision is reviewed before enabling preview, gateway, OTLP, webhook, or outbound behavior", []string{"deployment smoke", "operator approval", "production enablement"}),
