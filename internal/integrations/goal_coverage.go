...
func goalCoverageSections(capabilities map[string]Capability) []GoalCoverageSection {
	sections := []GoalCoverageSection{
		...
		},
		{
			...
			CapabilityIDs: []string{"collector.claude", "collector.codex", "collector.openclaw", "collector.opencode", "collector.kiro", "collector.pi", "protocol.provider_profiles", "protocol.agent_profiles", "protocol.signal_taxonomy", "protocol.signal_coverage", "protocol.integration_readiness", "protocol.integration_smoke", "protocol.integration_compatibility", "protocol.integration_rollout_plan", "protocol.integration_evidence_kit", "protocol.integration_drift", "protocol.integration_recommendation", "protocol.opentelemetry_genai", "protocol.otlp_receiver", "protocol.a2a", "gateway.provider_api", "gateway.provider_live_proxy"},
			Evidence: GoalCoverageEvidence{
				Endpoints:    []string{"GET /api/provider-profiles", "GET /api/agent-profiles", "GET /api/signal-taxonomy", "GET /api/integrations/signal-coverage", "GET /api/integrations/readiness", "GET /api/integrations/smoke", "GET /api/integrations/compatibility", "GET /api/integrations/rollout-plan", "GET /api/integrations/evidence-kit", "GET /api/integrations/drift", "GET /api/integrations/recommendation", "POST /api/otel/genai", "POST /api/otlp/v1/traces", "POST /api/a2a/tasks", "POST /api/provider/calls", "POST /gateway/openai/v1/chat/completions", "POST /gateway/openai/v1/responses", "POST /gateway/anthropic/v1/messages"},
				Commands:     []string{"agent-ledger provider profiles", "agent-ledger agent profiles", "agent-ledger signals", "agent-ledger signal-coverage", "agent-ledger integrations readiness", "agent-ledger integrations smoke", "agent-ledger integrations compatibility", "agent-ledger integrations rollout-plan", "agent-ledger integrations evidence-kit", "agent-ledger integrations drift", "agent-ledger agent recommend", "agent-ledger adapter conformance", "agent-ledger otel convert", "agent-ledger a2a convert", "agent-ledger provider convert"},
				MCPTools:     []string{"ledger.provider_profiles", "ledger.agent_profiles", "ledger.signal_taxonomy", "ledger.signal_coverage", "ledger.integration_readiness", "ledger.integration_smoke", "ledger.integration_compatibility", "ledger.integration_rollout_plan", "ledger.integration_evidence_kit", "ledger.integration_drift", "ledger.integration_recommendation", "ledger.adapter_conformance"},
				MCPResources: []string{"agent-ledger://integrations/provider-profiles", "agent-ledger://integrations/agent-profiles", "agent-ledger://integrations/signal-taxonomy", "agent-ledger://integrations/signal-coverage", "agent-ledger://integrations/readiness", "agent-ledger://integrations/smoke", "agent-ledger://integrations/compatibility", "agent-ledger://integrations/rollout-plan", "agent-ledger://integrations/evidence-kit", "agent-ledger://integrations/drift", "agent-ledger://integrations/recommendation", "agent-ledger://integrations/adapter-contract"},
